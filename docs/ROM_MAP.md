...
| `03:9040` | *(6-entry rolling history + running sum)* | `LDX #$000c ; LDA $0ccd,X ; STA $0ccf,X ; ADC $00 ; STA $00 ; DEX DEX ; BNE` -- shifts a 6-slot 16-bit window up by one and accumulates, storing the total at `$0ccf`. A moving-average filter over `$0ccf`-`$0cdc`, updated per frame; which metric it smooths is not established |
